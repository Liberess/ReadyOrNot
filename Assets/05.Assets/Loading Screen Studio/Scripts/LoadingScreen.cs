    ...
    public class LoadingScreen : MonoBehaviour
    {
        ...
        public static void LoadScene(string sceneName)
        {
            try
            {
                ...
                instance.mySceneName = sceneName;

                if (fcgHelper == true) { sceneHelper = sceneName; }
        ...
        private void ProcessAsyncLoading()
        {
            ...
            else if (processLoading == true && enableVirtualLoading == true && forceCanvasGroup == false)
            {
                ...
                float value = Mathf.Round(progressBar.value * 100);
                if (value >= 100.0f)
                    statusObj.text = "100%";
                else
                    statusObj.text = value.ToString() + "%";
...
/*                            if (GameManager.Instance != null && !GameManager.Instance.IsPlay)
                    ...
                    else
                    {
                        ...
                        /*                        // Press Any Key를 하지 않았다면
                                                if (CampaignManager.Instance != null && !CampaignManager.Instance.IsPlay)
                                                    CampaignManager.Instance.GameStart();
